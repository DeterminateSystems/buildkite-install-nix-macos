...
steps:
  ...
  - label: "Test on $arch"
    ...
    command:
      ...
      - sudo bash -c 'set -x; sleep 1; launchctl unload /Library/LaunchDaemons/com.buildkite.buildkite-agent.plist && sudo launchctl load /Library/LaunchDaemons/com.buildkite.buildkite-agent.plist' &>/tmp/buildkite-restart.log & disown
EOF
